...
import numpy as np
import pylab as pl, scipy.optimize as op
...
def estimate_beta_parameters_newton(tau, nu, max_iter=5000, tol=1e-12):
    ...
    if __debug__:
        ...
        check_gradient(lambda eta: f_prime(eta)[0], lambda eta: f_hess(eta)[0], x0, tol=tol)
        check_gradient(lambda eta: f_prime(eta)[1], lambda eta: f_hess(eta)[1], x0, tol=tol)
    
    alpha0 = johnson_kotz_alpha_starting_point(log_p_bar)
    
    xopt, fopt, func_calls, grad_calls, hess_calls, warnflag = fmin_ncg(f, alpha0, f_prime, fhess=f_hess, full_output=True, disp=0, maxiter=max_iter)
    logging.debug(
        '\nxopt = %s\nfopt = %f\nfunc_calls = %d\ngrad_calls = %d\nhess_calls = %d\nwarnflag = %d',
    ...
    if not np.isfinite(xopt).all():
        raise RuntimeError('xopt not finite: %s\ntau = %s; nu = %f' % (xopt, tau, nu))
...
def plot_density_with_R(x, weights, filename, mixture_x=None, mixture=None, adjust=.05):
    ...
    if None != mixture_x:
        ...
        R['lines'](robjects.FloatVector(mixture_x), robjects.FloatVector(mixture), col="red")
    grdevices.dev_off()
...
def plot_digamma_differences():
    ...
    pl.imshow(digamma(a) - digamma(a + b), origin='lower', extent=[step,max,min,max])
    pl.colorbar()
...
def beta_conj_prior_log_partition(tau, nu):
    ...
    def f(eta0, eta1):
        result = np.exp(eta0 * tau[0] + eta1 * tau[1] - nu * betaln(eta0+1., eta1+1.))
        #if np.isinf(result):
...
def make_ll_fns(tau, nu):
    ...
    def ll_hess(x):
        ...
        rolled = np.rollaxis(eta_plus_one, -1)
        return (
            np.diag(eta_plus_one * (tau - nu * (digamma(rolled) - digamma(rolled.sum()))))
            - np.outer(eta_plus_one, eta_plus_one) * nu * (np.diag(polygamma(1, rolled)) - polygamma(1, rolled.sum(axis=0)))
        )
...
#        transform_ll_fns(ll, ll_prime, ll_hess)
    ...
    to_optimize = lambda x: -ll(x)
    to_optimize_prime = lambda x: -ll_prime(x)
    starts = (
        np.log(.5 * np.ones(2)),
        np.log(np.ones(2)),
        np.log(.1 * np.ones(2)),
...
#        check_gradient(transformed_ll, transformed_ll_prime, x0, tol=tol)
        ...
        check_gradient(lambda eta: ll_prime(eta)[0], lambda eta: ll_hess(eta)[0], x0, tol=tol)
        check_gradient(lambda eta: ll_prime(eta)[1], lambda eta: ll_hess(eta)[1], x0, tol=tol)
...
#        check_gradient(lambda x: transform_prime(x)[0,0], lambda x: transform_hess(x)[0,0], x0)
    ...
    logging.debug('%s made %d function calls and %d gradient calls', optimizer.__name__, func_calls, grad_calls)
...
class EtaPointEstimates(EtaDist):
    def __init__(self, K, exp_family):
        ...
        self.eta = np.random.lognormal(mean=0., sigma=3., size=(self.K, self.exp_family.dimension)) - 1.
...
#            self.eta[k] = self.ML_estimate2(mixture, mixture.q_z.params[:,k])
            ...
            self.eta[k], _bound = self.ML_estimate(mixture, mixture.q_z.params[:,k], starting_eta=self.eta[k])
#            diff = np.sqrt(((self.eta[k] - eta)**2).sum())
#            if diff > 1e-5:
#                raise RuntimeError('Different ML etas: %f; %s and %s' % (diff, eta, self.eta[k]))
            if __debug__:
                check_LL_increased(last_bound, mixture.variational_bound_piecewise(), 'Component %d' % k, tolerance=1e-5)
    ...
    def evaluate(self, T):
        ...
        for k in range(self.K):
            p[k] = np.exp(np.dot(T, self.eta[k]) - betaln(self.eta[k,0]+1., self.eta[k,1]+1.))
        assert (0. <= p).all()
        return p
    ...
    def ML_estimate(self, mixture, q_zk, starting_eta=None, use_transformed_ll=True):
        ...
        g, gprime, ghess = transform_fn_and_derivatives(ll_fn, h, ll_prime, hprime, ll_hess, hhess)
        ...
        if __debug__:
            ...
            # check first entry in hessians
            check_gradient(lambda eta: ll_prime(eta)[0], lambda eta: ll_hess(eta)[0], x0, tol=tol)
            check_gradient(lambda x: hprime(x)[0,1], lambda x: hhess(x)[0,1], x0)
            check_gradient(lambda x: hprime(x)[0,0], lambda x: hhess(x)[0,0], x0)
        ...
        if use_transformed_ll:
            to_optimize = lambda x: -g(x)
            to_optimize_prime = lambda x: -gprime(x)
            ...
            to_optimize = lambda eta: -ll_fn(eta)
            to_optimize_prime = lambda eta: -ll_prime(eta)
            starts = (
                np.array([0., 0.]),
            )
            if None != starting_eta:
...
class EtaFullDists(EtaDist):
    ...
    def update(self, mixture):
        self.tau[:] = mixture.tau + np.dot(mixture.q_z.params.T, mixture.weighted_x)
        ...
        for k in range(self.K):
            ...
            self.conj_prior_A[k] = beta_conj_prior_log_partition(self.tau[k], self.nu[k])
            
            # calculate expected eta
            self.exp_eta[k] = beta_conj_expected_eta(self.tau[k], self.nu[k], self.conj_prior_A[k])
            
            # calculate expected beta A
            self.exp_beta_A[k] = beta_conj_prior_expected_A(self.tau[k], self.nu[k], self.conj_prior_A[k])
    ...
    def predictive(self, num_points=None):
        ...
        for k in range(self.K):
            p[k] = beta_conj_prior_predictive_T(T, self.tau[k], self.nu[k], A=self.conj_prior_A[k])
        return X[:,0], p
    ...
    def evaluate(self, T):
        ...
        for k in range(self.K):
            for t, _T in enumerate(T):
                p[k,t] = np.exp(beta_conj_prior_log_partition(self.tau[k]+_T, self.nu[k]+1.) - self.conj_prior_A[k])
        assert (0. <= p).all()
        return p
...
class DirichletPiDist(object):
    ...
    def update(self, mixture):
        self.params[:] = np.dot(mixture.weights, mixture.q_z.params) + mixture.alpha
...
class StickBreakingPiDist(object):
    ...
    def update(self, mixture):
        self._update(np.dot(mixture.weights, mixture.q_z.params), mixture.alpha)
...
class StickBreakingVarDist(object):
    ...
    def __init__(self, E_counts, E_alpha):
        ...
        """
        self._pi_bar = BetaDist(E_counts + 1., greater_than(E_counts) + E_alpha)
        self._1_minus_pi_bar = BetaDist(self._pi_bar.b, self._pi_bar.a)
    ...
    K = property(lambda s: s._pi_bar.a.shape[0], None, None, 'Limit on components in variational distribution.')
    pi_bar = property(lambda s: s._pi_bar, None, None, 'Beta distribution over pi_bar.')
...
class ZDist(object):
    ...
    def _update_from_weights(self, weights):
        ...
        exp_weights[float('infinity') == exp_weights] = 1e300 # replace infinities with large numbers
...
#        self._update_from_weights(weights)
    ...
    def expected_LL(self, mixture):
        ...
        return (
            ...
            + approx_exp_log_gamma(mixture.alpha + weighted_params.sum(axis=0), V_z.sum(axis=0)).sum()
...
class ZIntegrateOutStickDist(ZDist):
    ...
    def update(self, mixture):
        ...
        log_z_ge = approx_exp_log(mixture.alpha + mixture.beta + z_slash_n_ge, V_z_slash_n_ge)
        log_z_l = less_than_2d(
            approx_exp_log(mixture.beta + z_slash_n_greater, V_z_slash_n_greater)
            - log_z_ge
        )
        exp_eta, exp_A = mixture.q_eta.expected_values()
        weights = (
    ...
    def expected_LL(self, mixture):
        ...
        return (
            approx_exp_log_beta(
                ...
                mixture.beta + greater_than_2d(weighted_params).sum(axis=0), greater_than_2d(V_z).sum(axis=0),
            )
...
class ExpFamilyMixture(object):
    ...
    def __init__(self, x, weights, K, exp_family, tau, nu, options):
        ...
        if options.point_estimates:
            logging.debug("Using point estimates for components' eta parameters.")
            self.q_eta = EtaPointEstimates(self.K, self.exp_family)
        else:
            logging.debug("Using full variational distribution for components' eta parameters.")
    ...
    def plot_ll(self, q_zk, max):
        ...
        eta = np.rollaxis(np.array(np.mgrid[step_size-1:max-1:step_size, step_size-1:max-1:step_size]), 0, 3)
    ...
    def variational_bound_piecewise(self):
        ...
        if hasattr(self, 'q_pi'):
            return np.array([
                self.q_eta.expected_LL(self),                           # part from q_eta 
                self.q_z.expected_LL(self),                             # part from z_n drawn from pi
                self.q_pi.expected_LL(self),                            # part from prior on pi - this is fixed as long as alpha=1.
                self.q_eta.entropy(self),                               # entropy of q_eta
                self.q_z.entropy(self),                                 # entropy of z
                self.q_pi.entropy(self),                                # entropy of q_pi
            ])
        else:
            return np.array([
                self.q_eta.expected_LL(self),                           # part from q_eta 
    ...
    def reorder_components(self):
        ...
        permutation = _permutation_by_sort(np.dot(self.weights, self.q_z.params))
    ...
    def update_to_convergence(self, eps=1e-4, min_iter=10, max_iter=50):
        test = LlConvergenceTest(eps=eps, should_increase=True, use_absolute_difference=True)
        last_bound = None
        for i in range(max_iter):
            if None == last_bound:
...
if '__main__' == __name__:
    ...
    for seed in range(1, 3):
        ...
        def test_exp_family():
            ...
            mixture = ExpFamilyMixture(T, options.K, exp_family, -np.ones(2), 1., options=options)
            ...
            def plot(iter):
                logging.info('Plotting distribution after iteration %d' % (iter))
                pl.figure()
                mixture.plot()
                pl.title('Mixture after iteration %d.' % (iter))
            
            # update mixture
            test = LlConvergenceTest(eps=options.tolerance, should_increase=True, use_absolute_difference=True)
