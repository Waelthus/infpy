...
def print_tree(tree, indent = 0):
    ...
    elif isinstance(tree, LeafNode): pass
    else: raise RuntimeError('Cannot print tree nodes of this type: ' + tree.__class__)
...
def visit_tree_nodes(tree, visitor):
    if None != tree:
        visitor(tree)
        if isinstance(tree, DecisionNode):
            for c in tree.children: visit_tree_nodes(c, visitor)
    return visitor
...
def mutate_node(node, context):
    ...
    elif isinstance(node, LeafNode): node.outcome = choice(context.outcomes)
    else: raise RuntimeError('Cannot mutate nodes of this type: ' + node.__class__)
...
def build_random_path(tree, path):
    ...
    path.append(tree)
    if isinstance(tree, DecisionNode): build_random_path(choice(tree.children), path)
    elif isinstance(tree, LeafNode): pass
    else: raise RuntimeError('Cannot build random path over nodes of this type: ' + tree.__class__)
...
def choose_random_decision_node(tree):
    ...
    if len(random_path) < 2: return (None, None)
    else:
...
def combine_trees(tree_1, tree_2):
    ...
    if None == parent_1 or None == parent_2: return False # don't do anything if we can't
...
def mutate_tree(tree, context, new_subtree_depth):
    ...
    if None != parent and None != node:
        ...
        parent.children[idx] = create_random_decision_tree(context, new_subtree_depth)
    ...
    if isinstance(tree, LeafNode):
        ...
        for c in tree.children:
            if None == tree_has_only_one_outcome(c, outcome):
                return None
        return outcome
    else: raise RuntimeError('Cannot find outcome of nodes of this type: ' + tree.__class__)
            ...
            if None != outcome:
                ...
                if isinstance(c, DecisionNode):
                    for rule, outcome in rules_satisfied:
                        only_outcome = only_possible_outcome(rule, outcome, c.rule)
                        if None != only_outcome:
                            tree.children[i] = c.children[only_outcome] # remove this level of decision making
