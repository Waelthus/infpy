...
class Population(object):
    ...
    def __init__(
        ...
        self.individuals = [ self.species.random_individual() for i in range(size) ]
    ...
    def score_individuals(self):
        for i in self.individuals: i.fitness = self.fitness_fn(i)

    def select_parents(self):
        self.individuals.sort(key=lambda x: x.fitness, reverse=not self.minimise)
    ...
    def reproduce(self):
        # how many children
        if self.carry_over_parents: num_children = len(self.individuals) - len(self.parents)
        else: num_children = len(self.individuals)

        from random import choice
        self.children = [ self.species.mate(choice(self.parents), choice(self.parents)) for i in range(num_children) ]
