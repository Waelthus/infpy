...
class VariationalDistribution(object):
    ...
    def _update_phi(self):
        ...
        sum_E_log_1_minus_Vj = array([exp_log_one_minus_v_i[:i].sum() for i in range(self.K)])
    ...
    def predictive(self, x):
        ...
        predictive_component = exp(
         [self.conj_prior.log_conjugate_predictive(x, self.tau[k]) for k in range(self.K)]
        )
...
def polya_urn(N, alpha):
    ...
    for n in range(N):
        ...
        multi_param = array([k==i and alpha or counts[i] for i in range(k+1)])/(n+alpha)
    ...
    class MvnTestCase(DpTestCase):
        ...
        def var_dist_info(self):
            ...
            for i in range(self.var_dist.K-1):
                stick_lengths[i] = p_v[i] * reduce(float.__mul__, (1. - p_v[j] for j in range(i)), 1.)
        ...
        def plot_phi(self):
            ...
            rects = bar(range(self.var_dist.K), self.var_dist.phi.sum(axis=0), color=DpTestCase.colours[:self.var_dist.K])
            title("Components")
